...
def data_process():
    ...
    def get_pinyin2text(words, words_py):
        # 获取同音词
        names_text = []
        for w, wp in zip(words, words_py):
            result = dag(dagparams, wp, path_num=2)
            if len(result) == 0:
                names_text.append([])
            else:
                for r in result:
                    if w not in r.path:
                        names_text.append(r.path)
                        break
        return names_text
    ...
    for ds in industry_ds:
        ...
        for ds_name in ds_names:
            if len(ds_name) == 0:
                continue
            ds_name = re.findall(pattern=r'[\u4e00-\u9fa5]+', string=ds_name)
            ds_name = ''.join(i for i in ds_name)
            ...
            words_py = get_text2pinyin(words)
            new_words = get_pinyin2text(words=words, words_py=words_py)
