...
model_trigger_path = f'models_save/model_trigger_crf2/'
model_role_path = f'models_save/model_role_crf2/'
...
def data_generator(path):
    ...
    for ds in dataset.readlines():
        ...
        for event in events:
            ...
            mask_trigger_label[triggers_pos[0]:triggers_pos[1]] = 1
            ...
            all_arguments.append((arguments, triggers_pos, event_type))
...
print(next(iter(data_generator(train_ds_path))))
...
def valid():
    ...
    def get_predict_spo(events):
        spo = []
        for event in events:
            arguments = event['arguments']
            for argument in arguments:
                role_event = argument['role']
                event = role_event.split('&')[0]
                role = role_event.split('&')[1]
    ...
    for ds in dataset.readlines():
        ...
        predicts = predict(text, model_trigger, model_role)

        predicts = get_predict_spo(predicts)
        len_lab += len(labels)
...
def fit():
    model_trigger = BertCrf4Trigger(output_dim=2)
    ...
    model_role = BertCrf4Role(output_dim=len(ids2seq)+1)
...
def get_predict_result():
    ...
    with open('duee_result_crf2.json', 'w', encoding='utf-8') as f:
        for ds in test_ds.readlines():
            ds = json.loads(ds)
            content = ds['text']
            valid_id = ds['id']
            predicts = predict(content, model_trigger, model_role)
            result = {'id': valid_id, 'event_list': []}
            for event in predicts:
                arguments = event['arguments']
                result_arguments = []
                for argument in arguments:
                    event_role = argument['role']
                    result_text = argument['argument']
                    result_event = event_role.split('&')[0]
                    result_role = event_role.split('&')[1]
                    result_argument = {'role': result_role, 'argument': result_text}
                    result_arguments.append(result_argument)
                events = {'event_type': result_event, 'arguments': result_arguments}
                result['event_list'].append(events)
