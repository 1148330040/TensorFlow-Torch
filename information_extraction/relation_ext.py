...
def data_generator(dataset):
    ...
    """
    ids, masks, tokens = [], [], []
    subject_labels, object_labels, positions, text_lengths = [], [], [], []
    ...
    for _, ds in dataset.iterrows():
        ...
        # mask4pre = np.zeros(( 2), dtype=np.int32)
        ...
        if len(ids) == batch_size or _ == len(dataset):
            yield {
                ...
                'text_lengths': np.array(text_lengths, dtype=np.int32)
            }
            ids, masks, tokens = [], [], []
...
class Model4Pre(tf.keras.Model):
    ...
    def __init__(self, predicate_sorts):
        ...
        self.obj_predict = tf.constant(1)


    @tf.function(input_signature=[(tf.TensorSpec([None, max_len, 768], name='hidden', dtype=tf.float32),
                                   tf.TensorSpec([None, 128, 1536], name='trainable_weights', dtype=tf.float32))])
    def call(self, inputs):
...
# model_sub = tf.saved_model.load('models_save/model_sub/best_model/current_new_loss_new_lr/9/')
# model_obj = tf.saved_model.load('models_save/model_obj/best_model/current_new_loss_new_lr/9/')
def predict(content):
    """预测函数, 步骤如下：
    1: 预测subject
...
def fit_step():
    ...
    def fit_models(inputs_model, inputs_labels, inputs_other):
        positions_sub, positions_obj, text_lengths = inputs_other
    ...
    for num in range(0, 15):
        ...
        for _, data in enumerate(dataset):
            ...
            p_sub, p_obj, loss_sub, loss_obj = fit_models(
                ...
                inputs_other=[sub_positions, obj_positions, text_lengths]
            )
