...
class Bert4student(tf.keras.Model):
    def __init__(self, output_dim):
        ...
        self.bert = TFBertModel.from_pretrained('uer/chinese_roberta_L-4_H-768', output_hidden_states=True)
...
path = ['student_save_merge/best_model_merge_4_last/', 'student_save_merge/mid_model_merge_4_last/', 'student_save_merge/mid_model_merge_4_epoch5_last/']

# skip指stu模型和tea模型进行层跳跃蒸馏
# 比如stu的第二层和tea的第2*n层蒸馏
# last指的是stu和tea的最后的连续n层蒸馏
# 比如stu是4层tea是12层
# 那么stu和tea的8,9,10,11层进行蒸馏

# skip_3_best: 0.8505813544366723
# skip_3_mid_last: 0.773256549691868
# skip_3_mid_epoch_5: 0.7811

# last_best: 0.8465977876061961
# last_mid_best: 0.7940284347129855
# last_mid_epoch_5: 0.7937872601164895
...
def fit():
    ...
    def fit_step(inputs, target):
        ...
        for var in student_model.trainable_variables:
            if 'attention' in var.name:
                ...
                stu_var_num = int(stu_var_num) + 8
                # stu_var_num = int(stu_var_num ) + 8     --------last
                # stu_var_num = int(stu_var_num) * 3 + 1  --------skip
    ...
    for num in range(10):
        ...
        student_model.save('student_save_merge/mid_model_merge_4_last/')

        if num == 4:
            student_model.save('student_save_merge/mid_model_merge_4_epoch5_last/')

        f1_stu = valid_process(path='student_save_merge/mid_model_merge_4_last/')
        ...
        if f1_stu > f1_value_start_stu:
            ...
            teacher_model.save('student_save_merge/best_model_merge_4_last/')
